...
# Copy from https://github.com/pytorch/tutorials/blob/master/beginner_source/aws_distributed_training_tutorial.py
# This is the code runs in pytorch aws trainer setup. https://pytorch.org/tutorials/beginner/aws_distributed_training_tutorial.html
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # disable below code to avoid GPU.
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            # comment out below cuda code to avoid GPU
            # input = input.cuda(non_blocking=True)
            # target = target.cuda(non_blocking=True)
...
dist.init_process_group(backend=dist_backend, init_method="env://", rank=int(sys.argv[1]), world_size=int(sys.argv[2]))
