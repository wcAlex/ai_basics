...
if 'MASTER_ADDR' in os.environ and os.getenv('MASTER_ADDR') is not None:
    master_address = os.environ['MASTER_ADDR']
else:
    os.environ['MASTER_ADDR'] = 'localhost'

master_port = '12355'
if 'MASTER_PORT' in os.environ and os.getenv('MASTER_PORT') is not None:
...
if 'MODEL_PATH' in os.environ and os.getenv('MODEL_PATH') is not None:
    master_port = os.getenv('MODEL_PATH')
...
# ----------------
#
#    To get this code running, we need provide two parameters, first parameter is global rank, second parameter is world size.
#    There are three environment variables need to be set as well: MASTER_ADDR, MASTER_PORT, DATA_PATH, MODEL_PATH
#       ex: export DATA_PATH=/home/chi/workspace/ai_basics/pytorch/data
#
# -  On the first terminal for node0, run ``$ python restnet_dist_train_cpu.py 0 4``
#
# -  On the second terminal for node0 run ``$ python restnet_dist_train_cpu.py 1 4``
#
# -  On the first terminal for node1, run ``$ python restnet_dist_train_cpu.py 2 4``
#
# -  On the second terminal for node1 run ``$ python restnet_dist_train_cpu.py 3 4``
#
